...
Desde el inicio del proyecto se decidió trabajar con dos motores de corriente continua que disponíamos: un motor Pittman con reductora de la serie GM9413 para la articulación 1 y un motor Faulhaber 2342L012CR para la articulación 2. En consecuencia, el diseño del manipulador 2R no partió de un dimensionamiento clásico ``par requerido $\rightarrow$ motor'', sino del problema inverso: ajustar la geometría, las masas y el rango de movimiento del mecanismo para que los motores disponibles pudieran cumplir la tarea de dibujo del trébol.

El motor Pittman con caja reductora \SI{19.7}{\colon\!1} presenta, según el catálogo del fabricante, un par continuo del orden de \SI{0.318}{\newton\metre} y un par de arranque cercano a \SI{0.77}{\newton\metre}. Estos valores resultan adecuados para la primera articulación, que soporta el peso principal del eslabón 1 y de la estructura que sostiene al motor 2. Para explotar este margen de par sin llevar al motor a la saturación térmica, se adoptaron tres decisiones de diseño: (i) limitar el rango angular útil de la articulación 1 a aproximadamente \SI{200}{\degree} (de \SI{-90}{\degree} a \SI{110}{\degree}), evitando configuraciones extremas con palanca desfavorable; (ii) utilizar acrílico de \SI{5}{\milli\metre} en el eslabón 1 para reducir la masa; y (iii) apoyar el eslabón en dos pares con chumaceras tipo pedestal de manera que la mayor parte de las cargas radiales se transmitan a los rodamientos y el motor trabaje principalmente aportando par al movimiento.

El motor Faulhaber 2342L012CR utilizado en la articulación 2 es un motorreductor de precisión de 12 V con caja reductora planetaria metálica, según la ficha comercial del motorreductor~[3], la etapa de engranajes con relación aproximada de $1\!:\!64$ entrega un par nominal del orden de \SI{17.54}{kg\cdot cm}, equivalente a unos \SI{1.7}{\newton\metre}, con una velocidad sin carga cercana a \SI{120}{rpm} en el eje de salida. Este nivel de par, incluso considerando pérdidas y márgenes de seguridad, es muy superior al requerido para acelerar y sostener el eslabón 2 (tubo EMT aligerado) y el efector final, por lo que la segunda articulación trabaja lejos de su límite. La reductora planetaria reduce la alta velocidad del micromotor \emph{coreless} y aumenta el par disponible, mientras que el encoder acoplado al eje de salida permite cerrar el lazo de posición. Para integrar el conjunto al manipulador se diseñó un soporte en PETG que fija el cuerpo del motorreductor a la plataforma superior y un acople de aluminio que transmite el par al tubo EMT, manteniendo la alineación del eje y minimizando los momentos flectores sobre los rodamientos del motor.


El resultado de estas decisiones es un manipulador en el que la carga gravitacional y los esfuerzos dinámicos se encuentran dentro de la envolvente de operación segura de ambos motores. La estructura se ajusta a los actuadores disponibles en lugar de exigir motores de mayor tamaño, lo que refleja una estrategia de diseño orientada al aprovechamiento de recursos existentes sin sacrificar la funcionalidad del sistema.

\subsection{Diseño mecánico y selección de componentes}

\subsubsection{Materiales y método de elaboración}

El eslabón 1 se construyó a partir de dos placas de acrílico de \SI{5}{\milli\metre} de espesor, cortadas por láser según el contorno definido en CAD. Estas placas forman dos columnas paralelas que funcionan como largueros de una viga tipo ``portal''. Cada columna se apoya en dos chumaceras de pedestal atornilladas a una base de MDF, generando un apoyo doble a cada lado que garantiza la alineación y reparte las cargas entre cuatro rodamientos. Uno de los laterales de acrílico incorpora el acople mecánico al eje del motor Pittman, mientras el opuesto actúa como apoyo libre.

En la parte superior de las columnas se montó una plataforma impresa en 3D en material PETG, diseñada como una viga rígida que une las dos placas acrílicas. Sobre esta plataforma se atornilla el motor Faulhaber (articulación 2) mediante un soporte también impreso en PETG. El eslabón 2 está constituido por un tubo EMT modificado, soldado en los extremos a piezas macizas que facilitan el acople al eje del motor y al soporte de la herramienta. La herramienta de marcado (un marcador tipo plumón) se aloja en un acople específico impreso en PETG de alta densidad, que permite el ajuste del útil y asegura suficiente rigidez durante el trazado.

Todos los componentes impresos (plataformas, soportes de motor y acople de marcador) se fabricaron con un porcentaje de relleno alto para maximizar la resistencia mecánica y minimizar deformaciones elásticas durante el movimiento. La base del manipulador se construyó en MDF, lo que proporciona una superficie plana, fácil de mecanizar y suficientemente rígida para las cargas del experimento. La combinación de corte láser en acrílico, impresión 3D en PETG y elementos comerciales (tubo EMT, chumaceras y tornillería) permitió un proceso de fabricación rápido, repetible y compatible con el equipamiento habitual de un laboratorio académico.

\subsubsection{Selección y adaptación de motores}

Desde el punto de vista mecánico, el motor Pittman se montó con el eje en disposición horizontal, fijado a la base de MDF mediante un soporte impreso en PETG que envuelve el cuerpo del motor y transfiere las fuerzas de reacción a través de tornillos pasantes. El acople entre el eje del motor y la columna de acrílico se realizó mediante un cubo metálico con tornillos prisioneros, de modo que la unión pudiera desmontarse para mantenimiento o reajuste. La geometría del eslabón 1 se definió para que el centro de masa del conjunto (columnas, plataforma y motor 2) permaneciera lo más cerca posible del eje de rotación, reduciendo el par gravitacional máximo que debe suministrar M1.

El motor Faulhaber se fijó a la plataforma superior mediante un soporte también en PETG, que distribuye las cargas en la cara delantera del motor y evita que éste trabaje como elemento estructural. El acople de aluminio prolonga el eje y proporciona una interfaz cilíndrica donde se sujeta el tubo EMT mediante prisioneros. Esto permite reemplazar el tubo por otros de distinta longitud sin modificar el motor. La elección del tubo EMT como eslabón 2 obedece a un compromiso entre peso reducido, facilidad de soldadura y suficiente rigidez a flexión para la escala del manipulador.

\subsubsection{Selección de sensores}

Para la medición de posición angular en ambas articulaciones se emplearon discos ranurados de \SI{180}{} ranuras acoplados directamente a los ejes de los motores y sensores ópticos OPB800 de la serie W. Cada sensor OPB800 integra un diodo emisor infrarrojo y un fototransistor en configuración ranurada; el disco pasa por la ranura interrumpiendo el haz de luz y generando una señal digital adecuada para ser leída por el microcontrolador [opb800]. Con \SI{180}{} ranuras y detección de un flanco por ranura, la resolución resultante es de \SI{2}{\degree} por transición, valor que se utilizó explícitamente en la simulación y en el control.

La elección de este tipo de sensor se justifica por su simplicidad eléctrica (salida de colector abierto), su robustez mecánica y la facilidad de integración con discos fabricados por corte láser o impresión 3D. Aunque la resolución es modesta comparada con la de encoders incrementales industriales, resulta suficiente para demostrar el efecto de la cuantización en el dibujo del trébol y mantener el coste del prototipo bajo. En futuras versiones podría sustituirse por encoders de mayor densidad o por dispositivos absolutos si se requiere mayor precisión.
...
% Ejemplos: frecuencia, duración, modalidad (presencial/virtual), herramientas usadas, etc.
Completar con la descripción de cómo se organizaron las reuniones del grupo,
qué tan efectivas fueron y qué dificultades se presentaron.

\subsubsection*{Manejo de tiempos y recursos}
% Ejemplos: cronograma, cumplimiento de plazos, organización de tareas, uso de recursos del curso, etc.
Completar con la reflexión sobre el cumplimiento de fechas, distribución de
tareas, ajustes al cronograma y uso de recursos disponibles.

\subsubsection*{Discusión y verificación del avance}
% Ejemplos: hitos, revisiones internas, evidencias de avance, etc.
Completar explicando cómo se monitoreó el avance del proyecto, cómo se tomaron
decisiones y cómo se revisaron los resultados parciales.

\subsubsection*{Interacción con el docente facilitador}
% Ejemplos: espacios de retroalimentación, uso de las tutorías, claridad de las orientaciones, etc.
Completar describiendo cómo fue la comunicación con el docente, qué tan útil
resultó la retroalimentación y cómo influyó en el proyecto.
...
La parte mecánica estuvo a cargo de Nicolás Garzón y Jonatan Valero, mientras que la sección de
electrónica y control fue conformada por Juan Beltrán, Sergio Bolaños, David Pirateque y Óscar Siabato.
...
La colaboración con el docente facilitador fue especialmente relevante en la sección de
electrónica–control. Sus aportes se reflejaron en la orientación para la selección del microcontrolador,
en la identificación adecuada de la planta y en la elección del tipo de control más conveniente para el
mecanismo. Las sugerencias del docente llevaron a ajustar decisiones de diseño y a refinar la estrategia
de control, lo que se tradujo en un mejor desempeño del sistema y en una comprensión más profunda de los
fundamentos teóricos asociados.
