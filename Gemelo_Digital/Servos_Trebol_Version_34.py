...
# -- 6. Función para Exportar Datos --

def exportar_angulos(angulos_q1, angulos_q2, puntos_x, puntos_y, num_puntos_giro):
    """
    Exporta los ángulos q1 y q2 a archivos CSV y Excel
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Crear lista de fases
    fases = ['GIRO'] * num_puntos_giro + ['TREBOL'] * (len(angulos_q1) - num_puntos_giro)
    
    # Crear diccionario con los datos
    datos = {
        'Punto': range(1, len(angulos_q1) + 1),
        'Fase': fases,
        'q1_grados': [round(q, 4) for q in angulos_q1],
        'q2_grados': [round(q, 4) for q in angulos_q2],
        'x_mm': [round(x, 4) for x in puntos_x],
        'y_mm': [round(y, 4) for y in puntos_y]
    }
    
    # --- Exportar a CSV ---
    nombre_csv = f'angulos_robot_{timestamp}.csv'
    try:
        with open(nombre_csv, 'w', newline='', encoding='utf-8') as archivo_csv:
            escritor = csv.writer(archivo_csv)
            # Escribir encabezados
            escritor.writerow(datos.keys())
            # Escribir datos
            for i in range(len(angulos_q1)):
                escritor.writerow([
                    datos['Punto'][i],
                    datos['Fase'][i],
                    datos['q1_grados'][i],
        ...
        print(f"\n✓ Archivo CSV exportado: {nombre_csv}")
    except Exception as e:
        print(f"\n✗ Error al exportar CSV: {e}")
    
    # --- Exportar a Excel ---
    nombre_excel = f'angulos_robot_{timestamp}.xlsx'
    try:
        df = pd.DataFrame(datos)
        
        # Crear writer de Excel con múltiples hojas
        with pd.ExcelWriter(nombre_excel, engine='openpyxl') as writer:
            # Hoja principal con todos los datos
            df.to_excel(writer, sheet_name='Datos Completos', index=False)
            
            # Hoja solo con fase de GIRO
            df_giro = df[df['Fase'] == 'GIRO']
            df_giro.to_excel(writer, sheet_name='Fase GIRO', index=False)
            
            # Hoja solo con fase de TRÉBOL
            df_trebol = df[df['Fase'] == 'TREBOL']
            df_trebol.to_excel(writer, sheet_name='Fase TREBOL', index=False)
            ...
            estadisticas = {
                ...
                'Valor': [
                    len(angulos_q1),
                    num_puntos_giro,
                    len(angulos_q1) - num_puntos_giro,
                    round(min(angulos_q1), 4),
                    round(max(angulos_q1), 4),
                    round(max(angulos_q1) - min(angulos_q1), 4),
                    round(min(angulos_q2), 4),
                    round(max(angulos_q2), 4),
                    round(max(angulos_q2) - min(angulos_q2), 4)
                ]
            }
            df_stats = pd.DataFrame(estadisticas)
            df_stats.to_excel(writer, sheet_name='Estadísticas', index=False)
        
        print(f"✓ Archivo Excel exportado: {nombre_excel}")
        print(f"  - Hojas incluidas: Datos Completos, Fase GIRO, Fase TREBOL, Estadísticas")
    except Exception as e:
        print(f"\n✗ Error al exportar Excel: {e}")

# -- 7. Proceso Principal de Simulación --
...
# -- 8. Exportar datos antes de la visualización --
print("\n" + "="*60)
print("EXPORTANDO DATOS...")
print("="*60)
exportar_angulos(angulos_reales_q1, angulos_reales_q2, puntos_reales_x, puntos_reales_y, num_puntos_giro)

# -- 9. Visualización y Animación --
